...
import type { IconKey, SocialData } from "../_data/social.ts";

const ICON_MAP: Record<IconKey, VNode> = {
  github: (
    <svg
...
export default ({ social }: Lume.Data & SocialData) => (
  <ul>
    {social.map((media) => (
      <li key={media.name} className="w-8">
        <a href={media.url} title={media.name} target="_blank" rel="noreferrer">
          {ICON_MAP[media.iconKey]}
        </a>
      </li>
    ))}
  </ul>
